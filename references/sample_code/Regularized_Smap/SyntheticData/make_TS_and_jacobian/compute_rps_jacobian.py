...
def dX_dt(X):
    dydt = np.array([-X[s]*(np.sum(np.dot(A,X)[0,s])) + 1./np.sqrt(SystemSize) * d_Wiener[s,:] * np.sqrt(X[s]*(np.sum(np.dot(abs(A),X)[0,s]))) * HeavisideTheta(X[s]) for s in range(0,len(X))])
    return(dydt)

### Compute the analytical Jacobian
num_species = time_series.shape[1]
jacobian_matrix = open('jacobian_rps.txt', 'w')
