...
Surr_UIC <- function(df, effect_var, cause_var, numSurr, uic_surr_tbl_path, uic_plot_path) {
  ...
  uic_res <- uic.optimal(df, lib_var = effect_var, tar_var = cause_var, E = E_range, tau = 1, tp = tp_range, alpha = 0.05) # nolint
  print(paste("Optimal E for", effect_var, ":", uic_res$E[1] + 1))

  # Save UIC results
  write.csv(uic_res, paste0(
    uic_surr_tbl_path, "result/", cause_var, "_cause_", effect_var,
    "_UIC_result.csv"
  ))
  ...
  print(paste0("path : ", uic_plot_path, "uic/", cause_var, "_cause_", effect_var, "_UIC.tiff"))

  ## Generate seasonal surrogate data for effect_var
  effect_surr <- rEDM::make_surrogate_seasonal(df[[effect_var]], num_surr = numSurr)
  write.csv(effect_surr, paste0(
    uic_surr_tbl_path, "surrogate_dt/", cause_var, "_cause_", effect_var,
    "_surrogate_data.csv"
  ))
  ...
  write.csv(significant_UIC, paste0(
    uic_surr_tbl_path, cause_var, "_cause_", effect_var, "_uic_p_surr_res.csv"
  ))
  print(paste(
    "quantiles df: tp =", significant_UIC$tp,
  ...
  print(paste0("path : ", uic_plot_path, "uic_surr/", cause_var, "_cause_", effect_var, "_UIC_surr.tiff"))
}
...
reg_smap_func <- function(df, effect_var, cause_var, uic_surr_tbl_path, res_save_path, xmap_plot_path, fig_date_path) {
  print("Regularized S-map analysis --- start")
  ...
  coef_res_long <- coef_res_cols %>%
    pivot_longer(cols = c(c_effect, c_cause), names_to = "coef", values_to = "value")
  ...
  print("boxplot and save s-map figure --- done")
  print(paste0("path : ", fig_date_path, "smapFig/", "coefficients_", cause_var, "_causes_", effect_var, ".tiff"))
  ...
  print("plot real time change and save s-map figure --- done")
  print(paste0("path : ", fig_date_path, "smapFig/", "merged_", cause_var, "_causes_", effect_var, ".tiff"))
}
