...
library(doParallel)
library(foreach)
library(doRNG)  # random seedの設定のため
pacman::p_load(
  lubridate,
  tidyverse,
  ISOweek,
...
# packageVersion("macam") # v 0.1.4, 0.1.9
packageVersion("macamts") # v 0.1.4, 0.1.9
packageVersion("rUIC") # v 0.9.12, 0.9.14
# library(rEDM)
...
process_df <- function(df, cause_var, effect_var) {
  ...
  if (chosen_quantile == "quantile_95") {
    summary_signif_res_one <- data.frame(
      ...
      uic_rmse = selected_row$uic_rmse,
...
UIC_Smap_func <- function(df, effect_var, cause_var, numSurr) {
  ...
  tmp_res$Rsmap_rho <- rsmap_ridge$stats$rho
  tmp_res$Rsmap_mae <- rsmap_ridge$stats$mae
  tmp_res$Rsmap_rmse <- rsmap_ridge$stats$rmse
  ...
  print(tmp_res)
  # summary_signif_res <- rbind(summary_signif_res, tmp_res)
  return(tmp_res)
}
...
dt_ID <- read.csv(
  "data/oknw/oknwID_weather_11to19_merge_241205.csv"
)
dt_ID <- dt_ID %>%
...
# 並列化したい対象のベクトル
regions <- c("North","South","Middle","Naha", "Miyako", "Yaeyama")
...
for (region_name in regions) {
  ...
  for (cols in colnames(df_log)[2:18]) {
    print(cols)
    ...
    summary_res_one <- data.frame(
      ...
      summary_raw = sum(dt_ID[[paste0(cols,"_", region)]])
    )
    print(summary_res_one)
    if (sum(df_log[[cols]]) == 0) {
      df_log <- df_log[, colnames(df_log) != cols]
    }
    if (sum(df_log[[cols]] == 0) > length(df_log[[cols]]) * 0.8) {
      df_log <- df_log[, colnames(df_log) != cols]
    }
    if (ncol(df_log) < 5) {
      return(NULL)
  ...
  write.csv(summary_res,paste0(global_res_save_path, "/", region, "/", "table/dataframe_summary_",region,".csv"))
  ...
  summary_signif_res <- data.frame(
    ...
    Rsmap_rho = numeric(), # Regularized S-map結果のrho
    Rsmap_mae = numeric(), # Regularized S-map結果のmae
    Rsmap_rmse = numeric(), # Regularized S-map結果のrmse
  ...
  # 利用可能なコア数を設定（例: 全コア数-3）
  numCores <- parallel::detectCores() - 3
  cl <- makeCluster(numCores)
  registerDoParallel(cl)
  
  ID_names <- colnames(df_log)[2:length(df_log)]
  comb <- expand.grid(
    effect_var = ID_names,
    cause_var = ID_names,
    stringsAsFactors = FALSE
